...
function toRawViews(v: string): string {
  return String(parseInt(String(v).replace(/[.\s]/g, ""), 10) || 0);
}
...
export default function EditPostPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/posts").then((r) => r.json()).then((posts) => {
      const post = posts.find((p: { id: string }) => p.id === id);
      if (post) setForm({ ...post, views: toRawViews(post.views ?? "0") });
    });
  }, [id]);
  ...
  async function save() {
    ...
    await fetch(`/api/admin/posts/${id}`, {
      ...
      body: JSON.stringify({ ...form, views: formatViews(form.views) }),
    });
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold" style={{ color: "#1a5276" }}>Chỉnh sửa bài viết</h1>
        </div>
        <button
          onClick={save}
          disabled={saving}
        ...
        <aside className="space-y-4 xl:sticky xl:top-4 xl:self-start">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-xs font-semibold text-gray-500">Lượt xem</label>
                <input
                  type="number"
                  min={0}
                  value={form.views}
                  ...
                  placeholder="VD: 145738"
                />
                <p className="mt-1 text-xs text-gray-400">Hiện tại: {formatViews(form.views)}</p>
              </div>
              <div>
